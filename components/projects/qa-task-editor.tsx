...
export function QATaskEditor({ task, projectId, open, onOpenChange, onSave }: QATaskEditorProps) {
  ...
  const handleGenerateCriteria = async () => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        criterios_aceptacion: data.criteria,
      }))

      toast({
        title: "Criterios generados",
        description: "Puedes ajustarlos antes de guardar",
      })
    } catch (error: any) {
      console.error("[QA Task Editor] Error generating criteria", error)
      toast({
        title: "Error",
        description: error.message || "No fue posible generar criterios",
        variant: "destructive",
      })
    } finally {
  ...
  const handleImageDelete = async (imageUrl: string) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        imagenes: prev.imagenes?.filter(image => image.url !== imageUrl) || [],
      }))
  ...
  const handleImageUploadSuccess = (newImage: QAImage) => {
    ...
    toast({
      title: "Imagen subida",
      description: "Ya se agregó a la tarea",
    })
  }
